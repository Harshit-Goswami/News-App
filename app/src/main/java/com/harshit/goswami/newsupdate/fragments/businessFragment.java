...
public class businessFragment extends Fragment {
    RecyclerView recyclerViewBu;
    ArrayList<ItemModal> BuNewsList;
    NewsAdapter BuNadapter;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        recyclerViewBu =v.findViewById(R.id.recyclerbusiness);
        recyclerViewBu.setLayoutManager(new LinearLayoutManager(getContext()));
        BuNewsList =new ArrayList<>();
        BuNadapter =new NewsAdapter(BuNewsList,getContext());
        recyclerViewBu.setAdapter(BuNadapter);
    ...
    private void findNews() {
        ApiUtilities.getapiInterface().getCategoryNews("in",100,ApiKey,"business").enqueue(new Callback<MainModal>() {
            @Override
            public void onResponse(Call<MainModal> call, Response<MainModal> response) {
                if(response.isSuccessful()) {
                    BuNewsList.addAll(response.body().getArticles());
