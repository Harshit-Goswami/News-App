...
public class technologyFragment extends Fragment {
    RecyclerView recyclerViewTe;
    ArrayList<ItemModal> TeNewsList;
    NewsAdapter TeNadapter;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        recyclerViewTe =v.findViewById(R.id.technologyfragment);
        recyclerViewTe.setLayoutManager(new LinearLayoutManager(getContext()));
        TeNewsList =new ArrayList<>();
        TeNadapter =new NewsAdapter(TeNewsList,getContext());
        recyclerViewTe.setAdapter(TeNadapter);
    ...
    private void findNews() {
        ApiUtilities.getapiInterface().getCategoryNews("in",100,ApiKey,"technology").enqueue(new Callback<MainModal>() {
            @Override
            public void onResponse(Call<MainModal> call, Response<MainModal> response) {
                if(response.isSuccessful()) {
                    TeNewsList.addAll(response.body().getArticles());
